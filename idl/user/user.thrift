...
struct ListUserModelsResponse {
    1: required list<UserModelDetail> list
...
struct UserModelDetail {
    ...
    7: required string provider_name
    8: optional i64 meta_id
    9: optional string default_params     // JSON string
    10: optional string config_json        // JSON string
    11: required i32 scope
    12: required i32 status
    13: required i64 created_at
    14: required i64 updated_at
    15: required bool has_secret          // 是否已配置密钥
    16: optional string secret_hint       // 密钥脱敏提示
    17: required i32 is_default           // 是否为默认（0=不是, 1=是）
}
...
service UserService {
    // 1. 创建用户模型
       CreateUserModelResponse CreateUserModel(1: CreateUserModelRequest request) (
           api.post="/api/user/model/create",
           api.category="user",
           api.gen_path="user"
       )
       ...
       // 13. 忘记密码
       EmptyResponse ForgotPassword(1: ForgotPasswordRequest request) (
       ...
       // 14. 重置密码
       BaseResponse ResetPassword(1: ResetPasswordRequest request) (
       ...
       // 15. 退出登录
       BaseResponse Logout(1: EmptyRequest request) (
